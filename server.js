#!/usr/bin/env node
// ============================================================
// stockflow-mcp — MCP server for the Stockflow.media catalog
//
// Search 18,000+ royalty-free stock images & 4K videos, drop the
// watermarked preview URLs straight into presentations, videos or
// designs, and hand the user the license page when they want the
// full-resolution file (from $1, pay once use forever).
//
// Data source: the public, statically-published search index at
// https://stockflow.media/data/search-index.json (refreshed on
// every catalog sync). No API keys needed.
// ============================================================
...
const LICENSE_URL = "https://help.stockflow.media/license/";
...
function scoreAsset(a, tokens) {
  ...
  for (const tok of tokens) {
    if (!hay.includes(tok)) return 0;      // AND semantics — every token must hit
    score += (a.t.toLowerCase().includes(tok) ? 3 : 1);
  }
  return score;
}

const server = new McpServer({ name: "stockflow", version: "0.1.0" });

server.tool(
  "search_assets",
  "Search Stockflow.media's royalty-free catalog (18,000+ 8K stock images and 4K video clips). " +
  "Returns watermarked preview URLs that can be used directly in presentations, videos, mockups " +
  "and designs, plus the license page where the user can buy the full-resolution file (from $1).",
  {
    ...
    category: z.string().optional().describe("Optional category/collection filter, e.g. 'Background' or 'Microscopic'"),
...
server.tool(
  "download_preview",
  "Download an asset's watermarked preview file to a local path so it can be embedded in a presentation, " +
  "video project or design draft. For the full-resolution unwatermarked file, the user must license it at the asset's license_page.",
  {
    id: z.string().describe("The asset's File_ID"),
    directory: z.string().describe("Absolute local directory to save into"),
  },
  async ({ id, directory }) => {
    const idx = await getIndex();
    const a = idx.find((x) => x.i === id.trim());
    ...
    if (!r.ok) return { content: [{ type: "text", text: `Preview download failed (HTTP ${r.status})` }], isError: true };
    const ext = path.extname(new URL(url).pathname) || (a.v ? ".mp4" : ".webp");
    const file = path.join(directory, `stockflow_${a.i}_preview${ext}`);
    ...
    return {
      content: [{
        ...
        text: JSON.stringify({
          ...
          title: a.t,
          watermarked: true,
          license_full_res_at: a.u,
          license_terms: LICENSE_URL,
        }, null, 2),
      }],
    };
  }
...
console.error("stockflow-mcp ready — catalog:", INDEX_URL);
